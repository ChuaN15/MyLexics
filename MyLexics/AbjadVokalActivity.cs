...
namespace MyLexics
{
	[Activity (Label = "AbjadVokalActivity",Theme = "@style/Theme.Menu", Icon = "@mipmap/icon", ScreenOrientation = ScreenOrientation.Landscape)]			
	public class AbjadVokalActivity : Activity
	{
		...
		protected override void OnCreate (Bundle savedInstanceState)
		{
			...
			SetContentView (Resource.Layout.AbjadVokal);

			loadBackgroundImage();
			loadButtonImage();

			horizontalSV = FindViewById<HorizontalScrollView>(Resource.Id.AbjadVokalhorizontalSV);
			...
			vokalList.Add(new vokalKonsonan("e2", Resource.Drawable.Abjad_Button_e2));
			...
			foreach (vokalKonsonan vk in vokalList)
			{
				if (firstSymbol == 1)// first button without left margin
				{
                    ...
                    imgButton.SetScaleType(ImageView.ScaleType.FitCenter);
			...
			FindViewById<ImageButton>(Resource.Id.AbjadVokalUIBackButton).Click += delegate
			{
			...
			FindViewById<ImageButton>(Resource.Id.AbjadVokalUILeftButton).Touch +=  delegate {
				horizontalSV.ScrollTo(Convert.ToInt32(horizontalSV.ScrollX) - 50, Convert.ToInt32(horizontalSV.ScrollY));
			};

			FindViewById<ImageButton>(Resource.Id.AbjadVokalUIRightButton).Touch += delegate
			{
				horizontalSV.ScrollTo(Convert.ToInt32(horizontalSV.ScrollX) + 50, Convert.ToInt32(horizontalSV.ScrollY));
			};
			...
			horizontalSV.ViewTreeObserver.ScrollChanged += delegate {
				...
				{
					FindViewById<ImageButton>(Resource.Id.AbjadVokalUIRightButton).Visibility = ViewStates.Invisible;
				}
				else if (horizontalSV.ScrollX == 0)
				{
					FindViewById<ImageButton>(Resource.Id.AbjadVokalUILeftButton).Visibility = ViewStates.Invisible;

				}
				else
				{
					FindViewById<ImageButton>(Resource.Id.AbjadVokalUILeftButton).Visibility = ViewStates.Visible;
					FindViewById<ImageButton>(Resource.Id.AbjadVokalUIRightButton).Visibility = ViewStates.Visible;
				}
			};
            ...
            FindViewById<ImageButton>(Resource.Id.AbjadVokalUIInfoButton).Click += delegate
            {
		...
		private async void loadBackgroundImage(){
			...
			linearLayout = FindViewById<LinearLayout>(Resource.Id.linearLayoutAbjadVokal);
		...
		private async void loadButtonImage()
		{
			...
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadVokalUIBackButton, drawableId = Resource.Drawable.Back_Button });
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadVokalUIInfoButton, drawableId = Resource.Drawable.Option_info });
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadVokalUIRightButton, drawableId = Resource.Drawable.arrowR });
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadVokalUILeftButton, drawableId = Resource.Drawable.arrowL });
			...
			options = await decodeImage.GetBitmapOptionsOfImageAsync(Resource.Drawable.Kenali_Vokal_Title, Resources);
			bitmapToDisplay = await decodeImage.LoadScaledDownBitmapForDisplayAsync(Resources, Resource.Drawable.Kenali_Vokal_Title, options, 250, 250);
			imgView = FindViewById<ImageView>(Resource.Id.AbjadVokalUITitle);
