...
namespace MyLexics
{
	[Activity(Label = "AbjadKonsonanBergambarActivity", Theme = "@style/Theme.Menu", Icon = "@mipmap/icon", ScreenOrientation = ScreenOrientation.Landscape)]
	public class AbjadKonsonanBergambarActivity : Activity
	{
		...
		protected override void OnCreate(Bundle savedInstanceState)
		{
			...
			// Create your application here
			SetContentView(Resource.Layout.AbjadKonsonanBergambar);

			loadBackgroundImage();
			loadButtonImage();

			horizontalSV = FindViewById<HorizontalScrollView>(Resource.Id.AbjadKonsonanBergambarhorizontalSV);
			...
			FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUIBackButton).Click += delegate
			{
			...
			FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUILeftButton).Touch += delegate
			{
				horizontalSV.ScrollTo(Convert.ToInt32(horizontalSV.ScrollX) - 50, Convert.ToInt32(horizontalSV.ScrollY));
			};

			FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUIRightButton).Touch += delegate
			{
				horizontalSV.ScrollTo(Convert.ToInt32(horizontalSV.ScrollX) + 50, Convert.ToInt32(horizontalSV.ScrollY));
			};
			...
			horizontalSV.ViewTreeObserver.ScrollChanged += delegate
			{
				...
				{
					FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUIRightButton).Visibility = ViewStates.Invisible;
				}
				else if (horizontalSV.ScrollX == 0)
				{
					FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUILeftButton).Visibility = ViewStates.Invisible;

				}
				else
				{
					FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUILeftButton).Visibility = ViewStates.Visible;
					FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUIRightButton).Visibility = ViewStates.Visible;
				}
			};
            ...
            FindViewById<ImageButton>(Resource.Id.AbjadKonsonanBergambarUIInfoButton).Click += delegate
            {
		...
		private async void loadBackgroundImage()
		{
			...
			linearLayout = FindViewById<LinearLayout>(Resource.Id.linearLayoutAbjadKonsonanBergambar);
		...
		private async void loadButtonImage()
		{
			...
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadKonsonanBergambarUIBackButton, drawableId = Resource.Drawable.Back_Button });
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadKonsonanBergambarUIInfoButton, drawableId = Resource.Drawable.Option_info });
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadKonsonanBergambarUIRightButton, drawableId = Resource.Drawable.arrowR });
			buttonResource.Add(new imageLoadingDetails { resource = Resource.Id.AbjadKonsonanBergambarUILeftButton, drawableId = Resource.Drawable.arrowL });
			...
			bitmapToDisplay = await decodeImage.LoadScaledDownBitmapForDisplayAsync(Resources, Resource.Drawable.Konsonan_Bergamabr_title, options, 250, 250);
